...
export async function getSignedUrl(c: Context) {
  try {
    ...
    const schoolId = c.get('schoolId') || c.req.param().schoolId;
    if (!schoolId) {
      throw new Error('schoolId not found in context, upload fails');
    }
    ...
    const safeFolder = `${folderName}/${schoolId}`.replace(/\/+$/, '');
...
// Endpoints
utilsRouter.post('/signed-url', adminAuthMiddleware, getSignedUrl);
utilsRouter.post('/make-public', adminAuthMiddleware, makePublic);
